...
void
write_average_kinetic_energy(const std::size_t lap, const std::string path, corgi::Grid<3>& grid)
{
  ...
  auto local_total_energy_n_number_of_particles = [&](const std::size_t particle_type) {
    ...
    auto total_energies = pic_tiles |
                          rv::transform([&](auto& tile) -> double { return tile.total_kinetic_energy(particle_type); });

    auto number_of_particles = pic_tiles |
                          rv::transform([&](auto& tile) -> double { return tile.number_of_particles(particle_type); });

      return std::tuple{std::reduce(total_energies.begin(), total_energies.end()),
  ...
  for(const auto n: rv::iota(0uz, num_species)) {
    ...
    total_energies[n] = E;
      ...
      if (grid.comm.rank() == 0) {
          MPI_Ireduce(MPI_IN_PLACE, &total_energies[n], 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD, &requests.back());
      } else {
      MPI_Ireduce(&total_energies[n], nullptr, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD, &requests.back());
      }
      ...
      if (grid.comm.rank() == 0) {
          MPI_Ireduce(MPI_IN_PLACE, &number_of_particles[n], 1, MPI_UINT64_T, MPI_SUM, 0, MPI_COMM_WORLD, &requests.back());
      } else {
          MPI_Ireduce(&number_of_particles[n], nullptr, 1, MPI_UINT64_T, MPI_SUM, 0, MPI_COMM_WORLD, &requests.back());
      }
  }

    auto statuses = std::vector<MPI_Status>(requests.size());
    if (MPI_SUCCESS != MPI_Waitall(requests.size(), requests.data(), statuses.data())) {
        throw std::runtime_error{"MPI reduction while calculating average energies failed!"};
    }
  ...
  if(grid.comm.rank() == 0) {
    std::fstream file { path, std::ios::out | std::ios::app };
      file << lap << ' ';
    for(const auto n: rv::iota(0uz, num_species)) {
        const auto e = number_of_particles[n] == 0uz ? 0.0 : total_energies[n] / static_cast<double>(number_of_particles[n]);
        file <<  e << ' ';
    }
    file << "\n";
  }
