...
class emf_antenna(unittest.TestCase):
    ...
    def test_antenna_mode_wave_vector_xyz(self):
        ...
        for i, j, k in self.get_index_space():
            ...
            self.assertAlmostEqual(Jx[i, j, k], cfl * (ky**2 + kz**2) * np.cos(phi_i), places=4, msg=f"{(i, j, k)}")
            self.assertAlmostEqual(Jy[i, j, k], cfl * -kx * ky * np.cos(phi_j), places=4, msg=f"{(i, j, k)}")
            self.assertAlmostEqual(Jz[i, j, k], cfl * -kx * kz * np.cos(phi_k), places=4, msg=f"{(i, j, k)}")


    def test_antenna_mode_mode_number_x(self):
        """
        A = (1, 2, 3) * exp(i * x * k)
        B = curl(A) = (0, -3, 2) * i * k * exp(i * x * k)
        J = cfl * curl(B) = cfl * (0, 2, 3) * k^2 * Re(exp(i * x * k))
        ...
        N = 2
        antenna = runko.emf.threeD.antenna_mode(A=(1, 2, 3), n=(N, 0, 0))
    ...
    def test_antenna_mode_wave_number_y(self):
        """
        A = (1, 2, 3) * exp(i * y * k)
        B = curl(A) = (3, 0, -1) * i * k * exp(i * y * k)
        J = cfl * curl(B) = cfl * (1, 0, 3) * k^2 * Re(exp(i * y * k))
        ...
        N = 3
        antenna = runko.emf.threeD.antenna_mode(A=(1, 2, 3), n=(0, N, 0))
        ...
        for i, j, k in self.get_index_space():
            ...
            self.assertAlmostEqual(Jx[i, j, k], cfl * K**2 * np.cos(K * yi), places=4, msg=f"{(i, j, k)}")
            self.assertAlmostEqual(Jy[i, j, k], 0)
            self.assertAlmostEqual(Jz[i, j, k], 3 * cfl * K**2 * np.cos(K * yk), places=4, msg=f"{(i, j, k)}")


    def test_antenna_mode_wave_number_z(self):
        """
        A = (1, 2, 3) * exp(i * z * k)
        B = curl(A) = (-2, 1, 0) * i * k * exp(i * y * k)
        J = cfl * curl(B) = cfl * (1, 2, 0) * k^2 * Re(exp(i * y * k))
        ...
        N = 1
        antenna = runko.emf.threeD.antenna_mode(A=(1, 2, 3), n=(0, 0, N))
    ...
    def test_antenna_mode_wave_number_xyz(self):
        """
        k = (kx, ky, kz)
        A = (1, 0, 0) * exp(i * dot(k, x))
        B = curl(A) = (0, kz, -ky) * i * exp(i * dot(k, x))
        ...
        Nx, Ny, Nz = 1, 3, 2
        antenna = runko.emf.threeD.antenna_mode(A=(1, 0, 0), n=(Nx, Ny, Nz))
    ...
    def test_antenna_modes_are_linear(self):
        """
        k = (kx, ky, kz)
        A = (1, 0, 0) * exp(i * dot(k, x))
        B = curl(A) = (0, kz, -ky) * i * exp(i * dot(k, x))
        ...
        for i, j, k in self.get_index_space():
            ...
            self.assertAlmostEqual(Jx[i, j, k], 3.5 * cfl * (ky**2 + kz**2) * np.cos(phi_i), places=4, msg=f"{(i, j, k)}")
            self.assertAlmostEqual(Jy[i, j, k], 3.5 * cfl * -kx * ky * np.cos(phi_j), places=4, msg=f"{(i, j, k)}")
            self.assertAlmostEqual(Jz[i, j, k], 3.5 * cfl * -kx * kz * np.cos(phi_k), places=4, msg=f"{(i, j, k)}")


    def test_antenna_modes_cancel_each_other(self):
        """
        J = cfl * curl(curl(A - A)) = 0
        """

        tile = self.make_test_tile()

        Nx, Ny, Nz = 1, 3, 2
        A = np.array((4.2, 2.2, -2.4))
        antenna = runko.emf.threeD.antenna_mode(A=A, n=(Nx, Ny, Nz))
        tile.register_antenna(antenna)
        antenna = runko.emf.threeD.antenna_mode(A=-A, n=(Nx, Ny, Nz))
        tile.register_antenna(antenna)
        ...
        self.assertTrue(np.all(np.isclose(Jx, 0, atol=1e-5)))
        self.assertTrue(np.all(np.isclose(Jy, 0, atol=1e-5)))
        self.assertTrue(np.all(np.isclose(Jz, 0, atol=1e-5)))


    def test_antenna_mode_error_checking(self):

        with self.assertRaises(Exception):
            runko.emf.threeD.antenna_mode(k=(1, 2, 2))


        with self.assertRaises(Exception):
            runko.emf.threeD.antenna_mode(n=(1, 2, 2))


        with self.assertRaises(Exception):
            runko.emf.threeD.antenna_mode(A=(1, 1, 1))


        with self.assertRaises(Exception):
            runko.emf.threeD.antenna_mode(A=(1, 1, 1), n=(1, 2, 2), k=(1, 1, 1))
