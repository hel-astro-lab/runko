...
// Based on Smilei
// Refs: https://github.com/SmileiPIC/Smilei/blob/master/src/DomainDecomposition/Hilbert_functions.cpp 

//Functions for manipulating Hilbert curves in an arbitrary 
//number of dimensions and arbitrary size.  Taken from Chris Hamilton, 
//Technical Report CS-2006-07, Faculty of computer Science, Halifax.
...
//Given a non-negative integer g, calculates the non-negative integer i 
//such that gc(i)=g.
...
// Tsb = trailing set bit. It is the number of trailing set bits in the 
...
// curve defined by e and d will map tot he standard binary reflected gc.
...
// simulation box with 2^m patches per side (2^(2*m) patches in total).
unsigned int hilbertindex(
    unsigned int m,
    unsigned int x,
    unsigned int y,
    unsigned int *einit,
...
//!Hilbert index3D calculates the Hilbert index h of a patch of coordinates x,y,z 
//for a simulation box with 2^m patches per side (2^(3*m) patches in total).
unsigned int hilbertindex(
    unsigned int m,
    unsigned int x,
    unsigned int y,
...
//!Hilbert index2D inv  calculates the coordinates x,y of the patch of Hilbert 
//index h in a simulation box with 2^m patches per side (2^(2*m) patches in total).
...
//!General Hilbert index2D calculates the  Hilbert index h of a patch of coordinates x,y 
//for a simulation box with 2^mi patches per side (2^(m0+m1)) patches in total).
unsigned int generalhilbertindex(
    unsigned int m0,
    unsigned int m1,
    int x,
    int y,
    ...
    )
{
  ...
  if( (x<0) || (x>=(1<<m0)) || (y<0) || (y>=(1<<m1)) ) assert(false);
    ...
    )
{
  ...
  if( (x<0) || (x>=(1<<m0)) || (y<0) || (y>=(1<<m1)) ) {
    std::cout << "x: " << x << " / " << (1<<m0) << "m0: " << m0 << "\n";
    std::cout << "y: " << y << " / " << (1<<m1) << "m1: " << m1 << "\n";
    assert(false);
  }
...
//!General Hilbert index3D calculates the compact Hilbert index h of a patch of 
//coordinates x,y,z for a simulation box with 2^mi patches per side (2^(m0+m1+m2)) 
//patches in total).
unsigned int generalhilbertindex(
    unsigned int m0,
    unsigned int m1,
...
{
  if( (x<0) || (x>=(1<<m0)) || (y<0) || (y>=(1<<m1)) || (z<0) || (z>=(1<<m2))) assert(false);
  ...
  //Now in a local cube of side mi[dimmin]. The local entry point 
  //"einit" and initial direction "dinit" of the local hilbert curve has been
...
// of a patch for a given Hilbert index h in a simulation box with 2^mi 
// patches per side (2^(m0+m1) patches in total)
//
// 2D version
    ...
    unsigned int h)
{
    ...
    //Localize in which sub hypercube the point is. Do not account for 
    //the first 3*dimmin bits of h.
    ...
    // We need to run the 3D inversion algorithm on this cube with the 
    // correct entry point and direction.
    // Run the 2D indexgenerator in order to evaluate e and d.
    localh = generalhilbertindex(
        mi[dimmax]-mi[dimmin],
        mi[dimmed]-mi[dimmin],
        *localp[dimmax],
        *localp[dimmed],
