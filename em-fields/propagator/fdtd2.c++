...
void fields::FDTD2<1>::push_e(fields::Tile<1>& tile)
{
  ...
  {
    // Ex NONE
    mesh.ey(i,j,k) += + C*( mesh.bz(i-1,0,0) - mesh.bz(i,0,0));
    mesh.ez(i,j,k) += + C*(-mesh.by(i-1,0,0) + mesh.by(i,0,0));
...
void fields::FDTD2<2>::push_e(fields::Tile<2>& tile)
{
  ...
  [=] DEVCALLABLE (int i, int j, YeeLattice &mesh)
  {
    ...
    mesh.ex(i,j,k) += + C*(-mesh.bz(i,  j-1,0) + mesh.bz(i,j,0));
    mesh.ey(i,j,k) += + C*( mesh.bz(i-1,j,  0) - mesh.bz(i,j,0));
    mesh.ez(i,j,k) += + C*( mesh.bx(i,  j-1,0) - mesh.bx(i,j,0) 
...
void fields::FDTD2<1>::push_half_b(fields::Tile<1>& tile)
{
  ...
  {
    // Bx NONE
    mesh.by(i,j,k) += + C*( mesh.ez(i+1,0,0) - mesh.ez(i,0,0));
    mesh.bz(i,j,k) += + C*(-mesh.ey(i+1,0,0) + mesh.ey(i,0,0));
  }, 
    tile.mesh_lengths[0], 
    mesh);
...
void fields::FDTD2<2>::push_half_b(fields::Tile<2>& tile)
{
  ...
  [=] DEVCALLABLE (int i, int j, YeeLattice &mesh)
  {
    mesh.bx(i,j,k) += + C*(-mesh.ez(i,  j+1,0) + mesh.ez(i,j,0));
    mesh.by(i,j,k) += + C*( mesh.ez(i+1,j,  0) - mesh.ez(i,j,0));
    mesh.bz(i,j,k) += + C*( mesh.ex(i,  j+1,0) - mesh.ex(i,j,0)
