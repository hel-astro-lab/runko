...
class Timer:
    ...
    def stats(self, name):
        ...
        if self.do_print:
            if t0 < 1.0e-6:
                print("--- {:.20}   time: {:8.5f} mus/{:3d}  ({})".format(name.ljust(20), t0/1.0e6,  cnts, t0))
            elif 1.0e-6 < t0 < 1.0e-3:
                print("--- {:.20}   time: {:8.5f} ms /{:3d}  ({})".format(name.ljust(20), t0/1.0e3, cnts, t0))
            elif 1.0e-3 < t0:
                print("--- {:.20}   time: {:8.5f} s  /{:3d}  ({})".format(name.ljust(20), t0,       cnts, t0))
    ...
    def comp_stats(self):
        ...
        for name in self.components:
            ...
            if self.do_print:
                t0 = tavg
                if t0 < 1.0e-5:
                    print("--- {:.20}   {:6.3f}%  |  time: {:8.5f} mus/{:3d}  ({:.8})".format(name.ljust(20), relt, t0*1.0e6, cnts, t0))
                elif 1.0e-5 < t0 < 1.0e-2:
                    print("--- {:.20}   {:6.3f}%  |  time: {:8.5f} ms /{:3d}  ({:.8})".format(name.ljust(20), relt, t0*1.0e3, cnts, t0))
                elif 1.0e-2 < t0:
                    print("--- {:.20}   {:6.3f}%  |  time: {:8.5f} s  /{:3d}  ({:.8})".format(name.ljust(20), relt, t0,       cnts, t0))
