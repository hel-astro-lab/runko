    ...
    pic::Tile<D>& tile)
{
  ...
  for(int n=n1; n<n2; n++) {
    ...
    // apply pressure
    vel[0][n] = static_cast<float_p>( u0*cinv + pressx );
    vel[1][n] = static_cast<float_p>( v0*cinv + pressy );
    vel[2][n] = static_cast<float_p>( w0*cinv + pressz );
