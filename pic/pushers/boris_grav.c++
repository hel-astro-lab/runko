    ...
    pic::Tile<D>& tile)
{
  ...
  for(int n=n1; n<n2; n++) {
    ...
    // apply pressure
    vel[0][n] = static_cast<real_prtcl>( u0*cinv + gravx );
    vel[1][n] = static_cast<real_prtcl>( v0*cinv + gravy );
    vel[2][n] = static_cast<real_prtcl>( w0*cinv + gravz );
