    ...
    pic::Tile<D>& tile)
{
  ...
  for(auto&& con : tile.containers) {
    ...
    for(size_t n=0; n<con.size(); n++) {
      ...
      // Sokolov
      con.ex(n) = compute( &cxd[2], &cyp[1], &czp[1], yee.ex, iy,iz,  id,jp,kp); // Ex(d,p,p)
      con.ey(n) = compute( &cxp[1], &cyd[2], &czp[1], yee.ey, iy,iz,  ip,jd,kp); // Ey(p,d,p)
      con.ez(n) = compute( &cxp[1], &cyp[1], &czd[2], yee.ez, iy,iz,  ip,jp,kd); // Ez(p,p,d)
      con.bx(n) = compute( &cxp[1], &cyd[2], &czd[2], yee.bx, iy,iz,  ip,jd,kd); // Bx(p,d,d)
      con.by(n) = compute( &cxd[2], &cyp[1], &czd[2], yee.by, iy,iz,  id,jp,kd); // By(d,p,d)
      con.bz(n) = compute( &cxd[2], &cyd[2], &czp[1], yee.bz, iy,iz,  id,jd,kp); // Bz(d,d,p)
