...
// Due to returned lambdas the return type has to be deduced and
// this can not be in its own compilation unit, i.e. this has to be in a header.
// To not make yee_lattice.h too long the interpolators
// are in own separate headers which are included at the end of yee_lattice.h.
...
inline runko::EB_interpolator<YeeLattice::value_type> auto
  YeeLattice::interpolate_EB_linear_1st(
    const std::array<value_type, 3> lattice_origo_coordinates) const
{
  // Note that these include the halo.
  const auto Emds = E_.mds();
  const auto Bmds = B_.mds();

  return [=](const toolbox::Vec3<value_type> pos) {
    ...
    for(const auto [ic, jc, kc]: tyvi::sstd::index_space(Ex_means_mds)) {
      const auto [ii, jj, kk] =
        std::array<std::size_t, 3> { static_cast<runko::index_t>(i + ic),
    ...
    auto eb = interpolated_EB {};

    eb.E[0] = lerp3D(delta_pos.data, Ex_means_mds);
    eb.E[1] = lerp3D(delta_pos.data, Ey_means_mds);
    eb.E[2] = lerp3D(delta_pos.data, Ez_means_mds);

    eb.B[0] = lerp3D(delta_pos.data, Bx_means_mds);
    eb.B[1] = lerp3D(delta_pos.data, By_means_mds);
    eb.B[2] = lerp3D(delta_pos.data, Bz_means_mds);
    return eb;
  };
}
