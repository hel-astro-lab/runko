#!/usr/bin/env python3
"""
Scan optimal tile/mesh configurations for 3D PIC turbulence simulations.

Enumerates valid (Nx, Ny, Nz, NxMesh, NyMesh, NzMesh) factorizations
for a cubic box, computes memory, load-balance, and GPU efficiency metrics,
and ranks configurations by a weighted combined score.

Usage:
    python scan_tile_configs.py --L 128 --n_gpus 8 --ppc 2
    python scan_tile_configs.py --L 160 --n_gpus 4 --ppc 2 --plot
"""
...
def load_balance_score(igrid, n_gpus):
    """Score load balance for uniform volumetric load; returns (score, worst_imbalance)."""
    tiles_per_rank = np.bincount(igrid.flatten(), minlength=n_gpus)
    total_tiles = np.sum(tiles_per_rank)
    ideal = total_tiles / n_gpus
    worst_imbalance = np.max(tiles_per_rank) / ideal
...
def scan_configs(args):
    ...
    print(f"Domain: {L} x {L} x {L} cells (cubic),  n_gpus = {n_gpus},  ppc = {ppc}")
    print()

    facts = enumerate_factorizations(L, min_mesh)
    n_facts = len(facts)

    print(f"Factorizations per axis: {n_facts},  total combos: {n_facts**3}")

    results = []
    n_skipped_tiles = 0
    n_skipped_mem   = 0

    # keep only (Ny,NyM) <= (Nz,NzM) to avoid duplicates in y-z
    configs = [(a, b, c) for a, b, c in product(facts, facts, facts) if b <= c]
    for (nx, nxm), (ny, nym), (nz, nzm) in tqdm(configs, desc="Scanning configs"):
        ...
        prtcl_per_tile = particle_bytes(nxm, nym, nzm, ppc, n_species)

        # early skip: lower-bound memory assuming perfect balance, zero virtual tiles
        mem_lower_bound = tiles_per_gpu * (emf_per_tile + prtcl_per_tile)
        ...
        rank_mem = (
            tiles_per_rank * (emf_per_tile + prtcl_per_tile)
            + vtiles * vemf_per_tile
        )
        ...
        bal_score, worst_imbal = load_balance_score(igrid, n_gpus)
...
def print_table(results, top=20):
    ...
    for i, r in enumerate(top_results, 1):
        print(
            ...
            f"{r['emf_MB']:7.2f} {r['prtcl_MB']:7.2f} | "
            f"{r['max_gpu_GB']:7.3f} | "
...
def main():
    p = argparse.ArgumentParser(description="Scan tile/mesh configs for PIC turbulence simulations")

    p.add_argument('--L', type=int, default=128, help='cubic box side length in cells (default: 128)')
